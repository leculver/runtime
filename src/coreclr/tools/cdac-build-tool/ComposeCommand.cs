...
internal sealed class ComposeCommand : Command
{
    ...
    private readonly Option<string?> baselineNameOption = new("--baseline-name") { Arity = ArgumentArity.ZeroOrOne, HelpName = "BASELINENAME", Description = "Baseline contract name (overrides the name embedded in object files)" };
    ...
    private async Task<int> Run(ParseResult parse, CancellationToken token = default)
    {
        ...
        var baselineName = parse.GetValue(baselineNameOption);
        var builder = new DataDescriptorModel.Builder(baselinesDir, baselineName);
